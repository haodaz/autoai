...
> **状态：✅ 核心已实现 + 🔧 持续迭代中**  
> **最后更新：2026-08-15**  
> **Git commit：`a7b2f23` feat(office): approval workflow + chat-to-task integration**
...
> [!NOTE]
> **Grace 硬编码问题**：✅ 用户同意保持现状。Grace 硬编码为最后一步，审批检查点插在 Grace 之前。

> [!NOTE]
> **两步流程的实现方式**：✅ 用户接受两步式。第一步可选"全自动"跳过第二步。

---

## Proposed Changes（✅ = 已实现，🔧 = 本轮修复）
...
> [!WARNING]
> **🔧 Bug fix (2026-08-15)**: 所有 Agent 路由（alice/bob/david/edda/eric/fiona）之前硬编码 `status: 'COMPLETED'`，
> 未检查 `requiresApproval`。已修复为 `task.requiresApproval ? 'AWAITING_APPROVAL' : 'COMPLETED'`。
...
**追加功能（2026-08-15）**：
- ✅ Step 1 新增"全自动"选项：不进入 Step 2，直接全速派发
- ✅ 支持从群聊 "转为任务" 自动填入内容（通过 `WorkspaceContext.pendingNewTaskInput`）
...
`executeAgent` 函数修改（[L261-302](file:///Users/aisandbox/Documents/myAI/src/app/(dashboard)/office/page.tsx#L261-L302)）：
- 执行完成后，检查该 Task 的 `requiresApproval`
- 如果 `true` → 前端状态设为 `awaiting_approval`，后端状态设为 `AWAITING_APPROVAL`
- 如果 `false` → 前端 `done`，后端 `COMPLETED`

**改动 2：Kanban 卡片新增"待审批"状态** ✅

- `awaiting_approval` 状态样式：**黄色/琥珀色边框**，显示 🟡 图标
- 卡片底部显示两个按钮：
  - "✏️ Copilot" → 打开 Copilot 查看/修改产物（编辑完后重新审批使用最新版本）
  - "✅ 批准" → 调用 `/api/bristh/approve`
...
**改动 4：自动恢复活跃管线** 🔧 _(2026-08-15 修复)_

- **问题**：用户切换到其他页面再回到 `/office`，正在进行的任务管线消失
- **修复**：新增 `useEffect` 在页面 mount 时调用 `GET /api/bristh/tasks?mode=history`，
  查找包含 `RUNNING` 或 `AWAITING_APPROVAL` 状态任务的 context，自动恢复看板视图
- **同时修复** `loadHistory` 的状态映射：之前只处理 `COMPLETED`/`FAILED`，
  现在正确映射所有状态 (`AWAITING_APPROVAL` → `awaiting_approval`, `APPROVED` → `done`, `RUNNING` → `working`, `PENDING` → `idle`)
