...
function extractStructuredFields(raw: Record<string, any>): Partial<TalentJournalEntry> {
  ...
  if (raw.scholar && typeof raw.scholar === 'object') {
    ...
    if (stats) {
      fields.h_index = stats.h_index;
      fields.cited_by_count = stats.cited_by_count ?? raw.scholar.cited_by_count;
      fields.works_count = stats.works_count ?? raw.scholar.works_count;
    } else {
      fields.h_index = raw.scholar.h_index;
      fields.cited_by_count = raw.scholar.cited_by_count;
      fields.works_count = raw.scholar.works_count;
    }
...
class TalentJournalManager {
  ...
  async saveTalentData(
    ...
    // 人才特征：平方人才ID、学术指标（H-index/引用数/论文数）、百科简介
    ...
    if (result?.error) {
      ...
      require('fs').appendFileSync('/Users/aisandbox/Documents/zhiji-yida/journal-debug.log', `[${new Date().toISOString()}] ERROR saveTalentData for ${talentName}: ${result.error}\n`);
      throw new Error(result.error);
    }
    if (statusNum !== undefined && statusNum !== 200) {
      console.error(`[TalentJournal] Save FAILED for ${talentName}: status=${statusNum}`);
      require('fs').appendFileSync('/Users/aisandbox/Documents/zhiji-yida/journal-debug.log', `[${new Date().toISOString()}] ERROR saveTalentData for ${talentName}: status=${statusNum}\n`);
      throw new Error(`Save failed with status ${statusNum}`);
    }
    
    require('fs').appendFileSync('/Users/aisandbox/Documents/zhiji-yida/journal-debug.log', `[${new Date().toISOString()}] SUCCESS saveTalentData for ${talentName}\n`);
  ...
  async updateEntry(
    ...
    const svcToken = getServiceToken() || token;
